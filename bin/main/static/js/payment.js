if (typeof(PAYMENT_JS) === 'undefined') {
    ...
    function fsubmit_requirement(f) {
        ...
        if (ptype === 'cmall') {
        	...
        	var msa_name_val = f.msa_name.value,
	            msa_phone_val = f.msa_phone.value,
	            msa_zipcode_val = f.msa_zipcode.value,
	            msa_address2_val = f.msa_address2.value,
	            payment_agree_val = f.payment_agree.checked;
        	
        	/* 배송 정보 */
            if( ! jQuery.trim(msa_name_val) ){
                alert("받는사람 이름을 입력해 주세요.");
                f.msa_name.select();
                return false;
            }
            
            if( ! jQuery.trim(msa_zipcode_val) ){
                alert("배송주소를 입력해 주세요.");
                f.msa_zipcode.select();
                return false;
            }
            
            if( ! jQuery.trim(msa_address2_val) ){
                alert("상세주소를 입력해 주세요.");
                f.msa_address2.select();
                return false;
            }
            
            if( !phone_regexp.test(msa_phone_val) ){
                alert("받는사람 휴대폰 번호를 올바르게 입력해 주세요.");
                f.msa_phone.select();
                return false;
            }
            
            if( ! payment_agree_val ){
                alert("해당 약관과 구매에 동의해 주세요.");
                f.payment_agree.select();
                return false;
            }
